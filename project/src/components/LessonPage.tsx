...
import { loadUserProgress, saveUserProgress, updateCourseProgress } from '@/utils/progressManager';
...
const LessonPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (courseId && lessonId) {
      const course = progress.courses.find(c => c.id === courseId);
  ...
  if (!courseId || !lessonId || !courseContent[courseId] || !userProgress) {
    return (
  ...
  const handleSubmitQuiz = () => {
    ...
    if (allCorrect && !quizCompleted) {
      ...
      const { updatedCourses, xpGained } = updateCourseProgress(
